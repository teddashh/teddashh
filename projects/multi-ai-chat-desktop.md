...
### Six guided presets, not one fan-out button
...
| Brainstorm | 12 rounds × four providers = 48 contributions, with rotating order across framing, divergence, cross-pollination, harvesting, and concept tests | Full-context idea development, a balanced concept portfolio, and concrete first experiments |

Brainstorm is the heaviest preset: its release notes recommend keeping all four provider sessions ready and allowing roughly 45–90 minutes. It compiles to the existing Free fan-out engine with dedicated prompt metadata rather than introducing a sixth workflow engine.
...
| Brainstorm | 12 輪 × 4 家 = 48 次貢獻；發言順序輪替，依序經過 framing、divergence、cross-pollination、harvesting、concept tests | 用完整上下文發展點子、整理均衡 concept portfolio，並產出具體的第一批實驗 |

Brainstorm 是最重的 preset；release notes 建議四家 provider 都保持 ready，並預留約 45–90 分鐘。它以專用 prompt metadata 編譯到既有 Free fan-out engine，而不是另造第六套 workflow engine。
