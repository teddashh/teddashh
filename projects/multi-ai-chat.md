...
- A dedicated response serializer now converts provider DOM into semantic Markdown. It preserves headings, emphasis, nested ordered and unordered lists, blockquotes, fenced code with safe fence sizing, tables, and HTTP(S) links while omitting controls, hidden nodes, scripts, styles, SVG, and media.
- Resumed conversations now build a safer bounded context: only the latest 16 replayable messages are considered, each message is capped at 2,500 characters, the combined context is capped at 12,000, and empty, streaming, or error placeholders are excluded. The wrapper tells providers to treat earlier assistant text as reference rather than system instructions.
- Transcript scrolling is now reader-aware. Streaming stays pinned only while the reader remains near the end; switching conversations or sending a new message deliberately returns to the newest content.
- ChatGPT rich-editor injection now validates the actual composer draft after the browser insertion event and rebuilds mismatched content before sending, preventing a stale or partial draft from being accepted as the requested prompt.
...
### 5. Isolate requests and cancellation
...
- 新增專用 response serializer，把 provider DOM 轉成有語意的 Markdown。標題、強調、巢狀有序／無序清單、引用、會安全調整 fence 長度的 code block、table 與 HTTP(S) link 都能保留；control、hidden node、script、style、SVG 與 media 則會略過。
- 重開 conversation 時會建立更安全的有界 context：只考慮最後 16 則可 replay message，每則最多 2,500 字元、合計最多 12,000 字元，並排除空白、streaming 或錯誤 placeholder。Wrapper 也明確要求 provider 把先前 assistant 內容視為參考，而非 system instruction。
- Transcript scrolling 會尊重閱讀位置：只有讀者仍在底部附近時才跟隨 streaming；切換 conversation 或送出新問題時，才會明確回到最新內容。
- ChatGPT rich-editor injection 現在會在 browser insertion event 後驗證 composer 的真實 draft；內容不符時先重建再送出，避免 stale 或殘缺 draft 被誤認為要求的 prompt。
...
### 5. Request isolation 與 cancellation
